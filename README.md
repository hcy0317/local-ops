...
- Windows 批处理任务可选择“使用管理员启动代理”；仅接受脚本选择器生成的结构化命令，不把手写 raw shell 交给高权限代理。
...
批处理任务在 Windows 上可以勾选“使用管理员启动代理”。该选项只接受通过文件选择器生成且已复核的 `.bat/.cmd`、`.ps1` 结构化脚本，或无参数的绝对本机 `.exe/.com`；手写 raw cmd/PowerShell、带参数 direct、legacy 文本、计划任务引用和 Docker 资源都会被拒绝。结构化脚本固定使用经过 owner/DACL/reparse 校验的系统 cmd 或 Windows PowerShell，不从任务目录、PATH 或 COMSPEC 解析解释器。每张卡片最多运行一个管理员任务，代理为其创建随机命名、kill-on-close 的独立 Job；任务自然退出后保留成功/失败退出码，手动中止显示“已中止”。交互代理锁定或控制器重启不会让运行状态不可查询，但再次启动或中止仍需当前浏览器重新解锁。代理退出会由 Job 自动结束仍运行的高权限子进程，避免孤儿；为了不让高权限进程跟随用户可替换日志路径写文件，管理员批处理当前只保存控制审计和退出结果，不采集脚本 stdout/stderr，也不支持保活。
