...
Installation is available only from a frozen Windows package. A source checkout returns `BROKER_PACKAGE_REQUIRED` so user-writable Python files cannot become the scheduled elevated entry.
