...
## 2026-08-24 local security hardening and live migration

- Scope: loopback caller authentication, browser-local elevation, protected broker program observation/stop, Limited persistent controller, broker-v3 scheduled-task fallback, non-inheritable instance lock, windowless Docker polling, and a Tailscale Serve identity-gateway session. No commit, push, PR or CI run is claimed for this worktree.
- Final Windows discovery: 289 tests `OK` in 42.780s with 15 explicit fixture/package gates skipped. Shared contracts: 34 `OK` with one non-admin symlink skip. Frontend plus HTTP hardening: 59 `OK`, including 16 HTTP security cases. Common project check: 12 checks passed, including 85 Python tests with one symlink skip and 37 Node tests. Docker resource/HTTP slice: 12/12 `OK`; HCY sidecar contracts: 3/3 `OK`.
- TDD regressions proved that a child created with inherited handles retained the old Mutex, a legacy Mutex blocked the replacement namespace, the Docker runner omitted `CREATE_NO_WINDOW`, the console-task installer started without waiting, source broker installation remained reachable, proxy identity could not bootstrap safely, and broker observation queried SID for unrelated process names. Each test failed first and passed after its bounded fix.
- Audited v6 controller archive: `dist/windows-security-fix-v6-20260824/local-ops-1.0.0-windows-x64-unsigned.zip`, SHA-256 `4c5337d88bcebedc09e35c2e98d7ef3437ff2a592b028234900d007cc0527257`; installed controller EXE SHA-256 `92d576f47b8716590898d91f16949769d0f08fb1194d737a7a24b1426642301c`; package smoke 1/1 in 23.331s. Audited v7 broker archive SHA-256 `6de97598b614b4122480a0a3566d12e342da2c582759e612d9b18649338030b8`; installed broker EXE SHA-256 `e882f747884a1b2c1f8ce814d321986b6c61727e92da3e8d9bc14833670b4344`; package smoke 1/1 in 21.901s.
- Live task: `\LocalOps-Console` is Running as `RunLevel Limited`, action `C:\Program Files\LocalOps\Broker\92d576f47b871659\LocalOps.exe --no-browser --preferred-port 9600`; listener PID 70208 on `127.0.0.1:9600`. The fixed Highest broker runs the separately verified v7 path `C:\Program Files\LocalOps\Broker\e882f747884a1b2c\LocalOps.exe`.
- Live authenticated state: `degraded=false`; 16 configuration cards are preserved, including 11 exact scheduled-task references with zero `missing`/unavailable rows, two Docker Compose references with control available, and three administrator-program favorites. Broker status reports installed/verified/running/unlocked with program stop and scheduled-task support. Headerless `/api/state` returns 403.
- Window regression: while eight authenticated `/api/state` requests exercised scheduled-task and Docker discovery over 21.466s, process-start sampling found no `cmd`, `conhost`, `schtasks` or `docker` process descended from Local Ops PID 85612. Observed console processes belonged to Docker Desktop's own `docker stats`, ASUS Armoury and StarRailCopilot, not Local Ops or a Guard definition.
- Tailnet check: `https://hcy-ops.long-antares.ts.net/api/state` returned 200 and issued one `Secure; HttpOnly; SameSite=Strict` cookie; a cookie-authenticated sensitive GET returned 200, a cross-site request returned 403, and a direct loopback request with a forged proxy bearer returned 421. The identity gateway remained healthy and its sidecar Tailscale container was not recreated.
- Broker performance check: before v7, alternating cold/hot remote states were 12.57–12.77s / 22–44ms. After name/path prefiltering, 12/12 requests returned 200 with `degraded=false` and 16 apps; cold states were 2.47–2.64s and hot states 22–34ms.
- Browser check: after 15 seconds of continuous remote polling, the connection rail read `已连接`, the disconnect banner had `aria-hidden=true` and `data-connection=up`, no degraded notice was visible, and all 16 cards rendered.
